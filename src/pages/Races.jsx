...
import { Reveal, PageHead, TabBar } from "../components/ui";
...
export default function Races() {
  ...
  return (
    <PageWrap>
      <PageHead
        kicker={`СЕЗОН ${state.season ?? "…"} · ФОРМУЛА-1`}
        title="Гонки"
        right={
          <div className="flex flex-wrap items-center gap-2">
            <select
              id="season"
              aria-label="Машина времени: сезон"
              value={season}
              onChange={(e) => go({ season: e.target.value })}
              className="rounded-lg border border-line bg-panel px-3 py-2 font-digits text-sm font-bold outline-none transition-colors focus:border-rosso"
            >
            ...
            {["1975", "2000", "2004", "2007", "2008"].map((y) => (
              <button
                key={y}
                onClick={() => go({ season: y })}
                title="Великий сезон Ferrari"
                className={`rounded-lg px-2.5 py-1.5 font-digits text-xs font-bold transition-colors ${
                  season === y ? "bg-giallo text-carbon" : "border border-line bg-panel2 text-dim hover:text-giallo"
                }`}
              >
                {y}
              </button>
      ...
      >
        ...
        <TabBar
          className="mt-6"
          tabs={TABS.filter(({ id }) => id !== "sim" || !isArchive)}
          active={tab}
