...
const BADGES = [
  { id: "lightning", icon: "⚡", title: "Молния", desc: "Реакция быстрее 250 мс", earned: () => bestReaction() != null && bestReaction() < 250 },
  { id: "champion-reflex", icon: "🏎️", title: "Рефлексы чемпиона", desc: "Реакция быстрее 200 мс", earned: () => bestReaction() != null && bestReaction() < 200 },
  { id: "pit-crew", icon: "🔧", title: "Пит-крю Маранелло", desc: "Пит-стоп быстрее 2.50 с", earned: () => bestPit() != null && bestPit() < 2.5 },
  { id: "world-record", icon: "🏆", title: "Мировой рекорд", desc: "Пит-стоп быстрее 1.82 с", earned: () => bestPit() != null && bestPit() < 1.82 },
  { id: "professor", icon: "🎓", title: "Профессор Маранелло", desc: "Викторина 20 из 20", earned: () => bestQuiz() === 20 },
  { id: "navigator", icon: "🗺️", title: "Штурман", desc: "Все 10 трасс угаданы", earned: () => bestTracks() === 10 },
  { id: "strategist", icon: "📋", title: "Стратег", desc: "3 подсчитанных прогноза", earned: () => scoredPredictions() >= 3 },
  {
    id: "collector",
    icon: "🧰",
    title: "Полный гараж",
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <span className={`text-2xl ${ok ? "" : "grayscale"}`}>{b.icon}</span>
              ...
              {ok && (
                <DownloadCardButton
                  ...
                  card={{ label: "Достижение открыто", value: b.icon, sub: `${b.title} — ${b.desc}` }}
                />
              )}
            </div>
          );
